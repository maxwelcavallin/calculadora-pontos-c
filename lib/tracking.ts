...
const SESSION_KEY = "viagente_calcc_session_id"
const FORM_ATTEMPT_KEY = "viagente_calcc_form_attempt_id"
const LEAD_EVENT_KEY = "viagente_calcc_lead_event_id"
...
export function getOrCreateSessionId(): string {
  ...
  const value = generateEventId("calcc_session")
...
export function resetFormAttemptId(): string {
  const value = generateEventId("calcc_attempt")
...
export function resetLeadEventId(): string {
  const value = generateEventId("calcc_lead")
 ...
 * NUNCA passar nome, e-mail, telefone ou valores exatos de pontos no `payload`.
 */
export function pushToDataLayer(eventName: string, payload: Record<string, unknown> = {}) {
  ...
  const enrichedPayload = {
    ...
    site_name: "viagente",
    tool_name: "calculadora_pontos_c",
    form_name: "calculadora_pontos_c",
    funnel_path: "calculadora-c",
  ...
  if (window.location.search.includes("debug_tracking=true")) {
    console.log("[Viagente Calculadora C Tracking]", eventName, enrichedPayload)
  }
}
